...
pkgs <- c("ggplot2", "dplyr", "readr", "plotly", "lubridate")
for(i in pkgs){
...
nla_sites <- read.csv("nla2007_sampledlakeinformation_20091113.csv")
...
# Do some manipulating with the newer dataset using diplyr
nla_sites_edit<- nla_sites %>%
  filter(VISIT_NO == 1) %>%
  select(SITE_ID,STATE_NAME,CNTYNAME) %>%
  rename_all(tolower) %>%
  mutate_if(is.character, tolower)
#View(nla_sites_edit)

#nla_sites_diplyr <- nla_sites %>%
